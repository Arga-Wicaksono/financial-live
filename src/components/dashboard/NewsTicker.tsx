...
export function NewsTicker() {
  ...
  useEffect(() => {
    fetchNews();
    intervalRef.current = setInterval(fetchNews, 120_000); // 2 min
  ...
  const content = items.map((item, idx) => (
    <span key={`${item.source}-${idx}`} className="inline-flex items-center gap-2 px-3 whitespace-nowrap">
      ...
      <a
        ...
        className="text-[11px] text-zinc-400 hover:text-white transition-colors duration-200 max-w-[280px] truncate"
        title={item.title}
      >
        {item.title}
      </a>
      <span className="text-[8px] text-zinc-700">{item.source}</span>
  ...
  return (
    <div className="shrink-0 overflow-hidden h-8 flex items-center bg-[#08080a] border-t border-zinc-800/30">
      ...
      <div className="flex items-center gap-1.5 px-3 shrink-0 bg-zinc-900/60 h-full border-r border-zinc-800/30">
        ...
        <span className="text-[9px] font-bold text-blue-400 tracking-[0.15em] uppercase">Berita</span>
      </div>
