...
function CryptoCard({ item, dir, tick }: { item: CryptoPairData; dir?: Direction; tick: number }) {
  ...
  return (
    <div className={`rounded-md px-3 py-2 flex flex-col justify-between transition-all duration-300 relative bg-zinc-900/40 border border-zinc-800/30 hover:border-zinc-700/50 h-full ${cardAnim}`}>
      {/* Top: Icon + Name + Change */}
      <div className="flex items-center justify-between gap-1">
        <div className="flex items-center gap-2 min-w-0">
          <div className="w-8 h-8 rounded-md bg-zinc-800/60 flex items-center justify-center shrink-0">
            <span className={`text-base leading-none ${iconColor}`}>{icon}</span>
          </div>
          <div className="min-w-0">
            <span className="font-bold text-white text-sm tracking-wide block truncate">{item.name}</span>
            <span className="text-zinc-600 text-xs font-normal block leading-none">/IDR</span>
          </div>
        </div>
        <div className={`px-2 py-0.5 rounded text-xs font-bold tabular-nums shrink-0 ${
      ...
      {/* Price — no padding wrapping, let it breathe */}
      <div className="my-1.5 min-w-0">
        <PriceCell
          ...
          format="currency"
          decimals={0}
          className="text-lg font-bold tabular-nums !px-0 !py-0"
          direction={dir}
        />
      </div>
      ...
      <div className="flex items-center justify-between text-xs gap-1">
        <span className="text-green-400/70 tabular-nums truncate min-w-0">
          B: {new Intl.NumberFormat('id-ID', { notation: 'compact', maximumFractionDigits: 1 }).format(item.buy)}
        </span>
        <span className="text-zinc-600 text-xs shrink-0">|</span>
        <span className="text-red-400/70 tabular-nums truncate min-w-0">
          J: {new Intl.NumberFormat('id-ID', { notation: 'compact', maximumFractionDigits: 1 }).format(item.sell)}
        </span>
      </div>
    ...
    <div className="h-full flex flex-col p-3 gap-2">
      ...
      <div className="flex items-center justify-between px-0.5 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-amber-400/90 tracking-widest uppercase">Crypto / IDR</span>
          <span className="text-xs text-zinc-700 font-mono">Indodax</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-1.5 h-1.5 rounded-full bg-green-500 live-dot-pulse" />
          <span className="text-xs text-zinc-600 font-mono">10s</span>
        </div>
      </div>
